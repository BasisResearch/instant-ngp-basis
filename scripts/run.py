...
def parse_args():
	...
	parser.add_argument("--djali", action="store_true", help="Run in Djali mode.")
...
# Here's a 4x3 camera matrix
[0.838669, 2.32831e-10, 0.544639, ]
[0.0789354, -0.98944, -0.12155, ]
[0.538887, 0.144932, -0.829813, ]
[0.109167, 0.338793, 2.0108, ]
# write a numpy function which takes as input a value in [0,1] and interpolates the bottom lef

def interpolate_matrix(val):
    # Original matrix
    matrix = np.array([
        [0.838669, 2.32831e-10, 0.544639],
        [0.0789354, -0.98944, -0.12155],
    ...
    # Ensure input value is in range [0, 1]
    # assert 0 <= val <= 1, "Input value must be in range [0, 1]"

    # Initial value of bottom left element
    initial_val = matrix[-1, 0]

    # Target value for interpolation
    target_val = 1.0

    # Interpolation
    new_val = initial_val * (1 - val) + target_val * val

    # Create a copy of the original matrix and replace the bottom left element
    matrix_new = matrix.copy()
    matrix_new[-1, 0] = new_val

    return matrix_new

x = 0
# Function to receive data from the C++ program
def receiveData(conn):
	...
	data = conn.recv(1024)
	# print("after rec ", x)
	cam_matrix = np.fromstring(data.decode('utf-8'), sep=' ')
	# print(cam_matrix)
	if cam_matrix.shape[0] == 12:
		cam_matrix = cam_matrix.reshape(4,3)
		# print(cam_matrix.shape, cam_matrix.dtype)
		x = cam_matrix[0,0]
		print("x = ", x)
		c2 = interpolate_matrix(-x)
		tosend = c2.transpose().copy()
		print("Sending", tosend)
		return (True, tosend)
	else:
		# print("Error: received data of incorrect shape")
		return (False, False)
...
def run_djali(snapshot):
	...
	if True:
		...
		while sw * sh > 1920 * 1080 * 4:
			...
			sh = int(sh / 2)
		second_window = False
		testbed.init_window(sw, sh, second_window=second_window)
		# if args.vr:
		# 	testbed.init_vr()
	...
	# tqdm_last_update = 0
	# print("!!!!!!!!!!!!!!!!!11n_steps is", n_steps, args.djali)
	return testbed

	if True:
		# threading.Thread(target=accept_connections, args=(testbed,), daemon=True).start()
		while True:
			testbed.frame()

# if __name__ == "__main__":
# 	snapshot = "/home/zenna/repos/Djali/instant-ngp/data/nerf/fox/base.ingp"
# 	run_djali(snapshot)
# 	args = parse_args()
...
# 			print("step ", testbed.training_step)
# 			while testbed.frame():
